...
uint64_t get_sar_operand_mask(int operand_size) {
    switch (operand_size) {
...
uint64_t get_sar_sign_mask(int operand_size) {
    switch (operand_size) {
...
void update_flags_sar(CPU_CONTEXT* ctx, uint64_t result, int operand_size, unsigned int count, bool carry_out) {
    ...
    set_flag(ctx, RFLAGS_SF, (result & get_sar_sign_mask(operand_size)) != 0);
...
void sar_rm(CPU_CONTEXT* ctx, uint8_t modrm, uint8_t sib, int32_t disp, uint64_t mem_addr, int operand_size, uint8_t raw_count) {
    ...
    uint64_t mask = get_sar_operand_mask(operand_size);
    uint64_t sign_mask = get_sar_sign_mask(operand_size);
