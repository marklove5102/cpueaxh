...
inline void execute_or_with_decoded(CPU_CONTEXT* ctx, const DecodedInstruction* inst_ptr) {
...
void execute_or(CPU_CONTEXT* ctx, uint8_t* code, size_t code_size) {
    ...
    execute_or_with_decoded(ctx, &inst);
}

inline void execute_or_fast(CPU_CONTEXT* ctx, const DecodedInst* dec) {
    decoded_inst_apply_prefix(ctx, dec);
    ctx->last_inst_size = dec->length;
    if (!decoded_inst_needs_mem_recompute(&dec->cached)) {
        execute_or_with_decoded(ctx, &dec->cached);
        return;
    }
    ...
    execute_or_with_decoded(ctx, &live);
}
