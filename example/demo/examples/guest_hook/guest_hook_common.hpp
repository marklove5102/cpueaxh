constexpr uint64_t GUEST_HOOK_CODE_BASE = 0x100000;
constexpr uint64_t GUEST_HOOK_STACK_BASE = 0x200000;
constexpr uint64_t GUEST_HOOK_STACK_SIZE = 0x1000;
constexpr uint64_t GUEST_HOOK_EXACT_ADDRESS = GUEST_HOOK_CODE_BASE + 6;
constexpr uint8_t GUEST_HOOK_CODE[] = {
    0x48, 0x89, 0xC8,
    0x48, 0x01, 0xD0,
    0x48, 0x83, 0xE8, 0x05,
    0x49, 0x89, 0xC0,
...
inline bool setup_guest_hook_demo(cpueaxh_engine** out_engine) {
    ...
    err = cpueaxh_mem_map(engine, GUEST_HOOK_CODE_BASE, 0x1000, CPUEAXH_PROT_READ | CPUEAXH_PROT_WRITE | CPUEAXH_PROT_EXEC);
    if (err != CPUEAXH_ERR_OK) {
    ...
    err = cpueaxh_mem_write(engine, GUEST_HOOK_CODE_BASE, GUEST_HOOK_CODE, sizeof(GUEST_HOOK_CODE));
    if (err != CPUEAXH_ERR_OK) {
    ...
    err = cpueaxh_mem_map(engine, GUEST_HOOK_STACK_BASE, GUEST_HOOK_STACK_SIZE, CPUEAXH_PROT_READ | CPUEAXH_PROT_WRITE);
    if (err != CPUEAXH_ERR_OK) {
    ...
    uint64_t value = GUEST_HOOK_STACK_BASE + GUEST_HOOK_STACK_SIZE - 0x80;
...
inline void run_guest_hook_demo_common(cpueaxh_engine* engine, const char* title) {
    print_context(engine, title);

    cpueaxh_err err = cpueaxh_emu_start(engine, GUEST_HOOK_CODE_BASE, GUEST_HOOK_CODE_BASE + sizeof(GUEST_HOOK_CODE), 0, 0);
    std::cout << title << " result: " << cpueaxh_err_string(err)
        << " (" << err << ")" << std::endl;

    print_context(engine, "after");
}
